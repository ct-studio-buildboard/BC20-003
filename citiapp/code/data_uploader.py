...
def upload_data(filename):
	...
	expected_cols = ['Event Name', 'PM/Contact', 'Analyst', 'Account Name',
			...
			'Open Exchange Indicator', 'Mgmt', 'Meeting Corporate Title']
	missing_cols = []
	...
	new_interactions = df.shape[0] - num_interactions
	...
	if new_interactions > 0:
		return "Success: {} new records added to the database".format(new_interactions)
	else:
		return """Warning: No new records were added.  Possible causes are
					that the records already existed in the database, 
					the interactions were not of type 'Non Deal Roadshow' 
					or 'Reverse Roadshow', or the company name was 'Industry Experts'"""
